...
function getCatalogIndex(entries) {
  if (!Array.isArray(entries) || entries.length === 0) return null;
  if (catalogIndexes.has(entries)) return catalogIndexes.get(entries);
  const index = {};
...
export function buildArmorInventory(profileResponse, { language = "zh-chs", catalogEntries = null } = {}) {
  ...
  const catalog = getCatalogIndex(catalogEntries);
